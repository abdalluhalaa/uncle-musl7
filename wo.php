...
<head>
    <meta charset="utf-8">
    <title>Uncle Musl7 -  My Appointments</title>
...
<nav class="navbar navbar-expand-lg bg-white navbar-light sticky-top py-0 pe-5">
                    ...
                    <?php if(!isset($_SESSION["logged_in"]) || $_SESSION["logged_in"] !== true){
                        ...
                        <?php if(!isset($_SESSION["logged_in"]) || $_SESSION["logged_in"] !== true){
                            ...
                            if ($role == 'worker' || $role == 'admin')
                                echo '<a href="wo.php" class="dropdown-item active">My Appointments</a>';
 ...
 <div class="container-fluid page-header py-5 mb-5 wow fadeIn" data-wow-delay="0.1s">
        <div class="container py-5">
            <h1 class="display-3 text-white animated slideInRight"> My Appointments</h1>
    ...
    if ($conn->connect_error) {
        ...
        $worker_email = $_SESSION['email'];
        $sqle = "SELECT role, phone FROM user WHERE email = '$worker_email'";
    ...
    header("location: 404.html");
        ...
        $worker_email = $_SESSION['email'];
        $sql = "SELECT id, user_email, date, time ,title,des,status FROM appointments WHERE worker_email = '$worker_email'";
        $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {

            if ($row['status'] == "Pending") {
                $s = "<a class='btn btn-primary px-4' href='reject_appointment.php?id=" . $row['id'] . "'>Reject</a> ";
                $n = "<a class='btn btn-primary px-4' href='accept_appointment.php?id=" . $row['id'] . "'>Accept</a> ";
            } 
            elseif ($row['status'] == "Accepted") {
                $current_date = date("Y-m-d");
                if ($current_date >= $row['date']) {
                $s = "<a class='btn btn-primary px-4' href='finish.php?id=" . $row['id'] . "'>Finish</a> ";
                $n = " ";
                }else
                {
                    $s = "";
            ...
            <div class="bg-light text-center h-100 p-5">
                ...
                <h6 class="mb-4">Email: ' . $row['user_email'] . '</h6>
                <hr class="w-200">
